...
const addRequest = async (req, res) => {
  ...
  const { availability, error: availablityError } = await isCarAvailable(
    car_id,
    start_date,
    end_date
  );
  if (availablityError)
    return res.status(401).json({ error: availablityError.message });
  if (!availability)
    return res
      .status(401)
    ...
    .insert([
      {
        ...
        renter_id: "9cc9210d-4530-40f8-a13d-bd2af6d05801",
        car_id: 34,
      },
    ])
    .select("*")
...
const handleRequest = async (req, res) => {
  ...
  if (status == "accepted") {
    const { data, error: err } = await Supabase.from("bookings")
      .insert([
        {
          start_date: returnedRequest.start_date,
