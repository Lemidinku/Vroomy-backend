...
const getMyRents = async (req, res) => {
    ...
    const rents = supabase.from('rents').select('* cars(*)').eq('renter_id', renter_id)
...
const rateARent = async (req, res) => {
    ...
    if (account_type == "renter") {
        ...
        if (renterSatisfaction.renter_satisfaction != null) {
                res.status(400).json({ error: "You have already rated this rent", ownerRating: renterSatisfaction })
            }
        ...
        const { data, error } = await supabase
            ...
            .eq('renter_id', user_id)
            .eq('status', "completed")
            .select("car_id, renter_satisfaction").single()
        ...
        const { data: updatedOwner, error: error4 } = await supabase
            ...
            .eq('id', car.owner_id).select('*').single()
        ...
        if (ownerSatisfaction.owner_satisfaction) {
                ...
                res.status(400).json({ error: "You have already rated this rent", renterRating: ownerSatisfaction })
            }
        ...
        const { data, error } = await supabase
            ...
            .eq('rent_id', rent_id)
            .eq('status', "completed")
            .select("car_id, owner_satisfaction, renter_id").single()
        ...
        const { data: updatedRenter, error: error2 } = await supabase
            ...
            .eq('id', data.renter_id).select('*').single()
